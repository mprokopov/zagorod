...
module SortHelper
  ...
  def sort_link(column, text=nil)
    ...
    if key == column
      if order.downcase == 'asc'
        icon, order = 'up.gif', 'desc'
      else
        icon, order = 'down.gif', 'asc'
      end
    else
      icon, order = nil, 'asc'
    end
    ...
    link_to(text, params) +
      (icon ? nbsp(2)+image_tag(File.join(@icons_dir,icon),:width=>'8',:height=>'3', :border=>'0', :align=>'absmiddle') : '')
  end
  ...
  def sort_header_tag(column, options = {})
    ...
    key, order = session[@sort_name][:key], session[@sort_name][:order]
    if key == column
      content_tag('span', nbsp(2)+sort_link(column, options[:title]), options)
    else
      sort_link(column, options[:title])
    end
  end
