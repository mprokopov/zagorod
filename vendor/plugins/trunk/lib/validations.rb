        ...
        validates_each(attrs, options) do |record, attr, value|
          unless value.blank?
            begin
              ...
              record.errors.add('image', "слишком мал, размеры не менее #{minimums[0]}x#{minimums[1]}") if ( img.rows < minimums[1] || img.columns < minimums[0] )
            rescue ::Magick::ImageMagickError
              record.errors.add('image', "неверное изображение")
            end
            img = nil
            GC.start
          end
